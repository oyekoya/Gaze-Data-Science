...
% C13 = C{13}; %model_target (hit node)
...
%     end;
    if ((C2(x)=='C')|(C2(x)=='W'))
        ...
        %if (strcmp(C4(x),'don_body_sfd.wrl') == 1) sfd_nodeid = strrep(C3(x),'b','et'); sfd_et_index = node_num; end;
        %if (strcmp(C4(x),'don_body_rdg.wrl') == 1) rdg_nodeid = strrep(C3(x),'b','et'); rdg_et_index = node_num; end;
    end;
end;
%y
% store eyes, head and body info for global coords calculation
